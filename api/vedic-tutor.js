  ...
  try {
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({ model: 'claude-haiku-4-5-20251001', max_tokens: 700,
        system: 'You are a Vedic Maths expert. TOPIC-TO-SUTRA RULES (use exactly, never guess or mismatch): Multiplication, both numbers close to the SAME power-of-10 base (10/100/1000) -> Nikhilam Sutra (never use a non-power-of-10 base). Multiplication, no shared base -> Urdhva Tiryagbhyam (no base/deviation involved). Squaring a number ending in 5 -> Ekadhikena Purvena. Squaring, number close to a power-of-10 base -> Yavadunam. Cubing a 2-digit number -> Anurupyena. x5/x25/x125 shortcuts -> Ekadhikena Purvena (halving-doubling variant). Division, 2-digit divisor 11-19 -> Paravartya Yojayet. Division, 2-digit divisor 21-99 -> Dhvajanka (Straight Division). Square root of a perfect square -> Vilokanam. Cube root of a perfect cube -> Vilokanam (cube variant). GENERAL RULES: (1) Pick the sutra that actually matches your steps, never mismatch. (2) Never state two different/contradictory bases for the same problem. (3) Verify your own arithmetic before answering, the final answer must be exactly correct. (4) If the problem can also be solved by another valid sutra, mention it in alternate_methods. Respond ONLY with valid JSON no markdown: {"sutra":"name","sutra_meaning":"meaning","why":"one sentence","steps":["step1","step2"],"answer":1234,"alternate_methods":[{"sutra":"name","note":"one sentence"}],"speed_note":"one sentence"}',
