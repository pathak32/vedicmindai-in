...
const PLANS = [
  ...
  },
  {
    ...
    annualPrice: '₹4,990',
    annualStrike: '₹18,000',
    annualSub: '= ₹416/month · 2 months FREE',
    discount: '67% off',
    monthlyPaise: 49900,
    annualPaise: 499000,
    usdMonthly: '$9',
  ...
  },
  {
    ...
    annualPrice: '₹9,990',
    annualStrike: '₹36,000',
    annualSub: '= ₹832/month · 2 months FREE',
    discount: '67% off',
    monthlyPaise: 99900,
    annualPaise: 999000,
    usdMonthly: '$15',
    ...
    <div style={{ position: 'relative', flex: '1 1 240px', maxWidth: 320 }}>
      ...
      <div style={{
        ...
        {disablePurchase && !isCurrent && !isFree ? (
          ...
          {/* ── Lifetime Access Banner — 200 seats only ── */}
          {(() => {
            const seatsLeft = Math.max(0, LIFETIME_START_DISPLAY - lifetimeClaimed);
            const pctClaimed = Math.min(100, (lifetimeClaimed / LIFETIME_TOTAL) * 100);
            ...
            return (
              <div style={{
                ...
                border: isSoldOut ? '1px solid #4B5563' : '1px solid rgba(245,158,11,0.4)',
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 16, marginBottom: 20 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10, flexWrap: 'wrap' }}>
                      <span style={{ background: 'rgba(245,158,11,0.15)', border: '1px solid rgba(245,158,11,0.35)', borderRadius: 20, padding: '3px 12px', fontSize: 11, fontWeight: 700, color: '#F59E0B', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                        🔒 Lifetime Access — Basic Plan
                      </span>
                      {!isSoldOut && (
                        <span style={{ background: 'rgba(239,68,68,0.15)', border: '1px solid rgba(239,68,68,0.4)', borderRadius: 20, padding: '3px 12px', fontSize: 11, fontWeight: 700, color: '#FCA5A5' }}>
                          ⚡ Only {seatsLeft} seats left
                        </span>
                      )}
                      {isSoldOut && (
                        <span style={{ background: 'rgba(107,114,128,0.2)', borderRadius: 20, padding: '3px 12px', fontSize: 11, fontWeight: 700, color: '#9CA3AF' }}>
                          Sold Out
                        </span>
                      )}
                    </div>
                    <h3 style={{ color: 'white', fontSize: 18, fontWeight: 800, margin: '0 0 6px' }}>
                      Pay once. Learn Vedic Maths forever.
                    </h3>
                    <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: 13, margin: '0 0 16px', lineHeight: 1.55 }}>
                      One-time payment. No monthly charges. All Basic features, permanently.
                      Limited to {LIFETIME_TOTAL} founding members only.
                    </p>
                    {/* Scarcity progress bar */}
                    <div style={{ marginBottom: 4 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                        <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)' }}>
                          {lifetimeClaimed} of {LIFETIME_TOTAL} seats claimed
                        </span>
                        <span style={{ fontSize: 11, color: isSoldOut ? '#9CA3AF' : '#FCA5A5', fontWeight: 600 }}>
                          {isSoldOut ? 'All seats taken' : `${seatsLeft} remaining`}
                        </span>
                      </div>
                      <div style={{ background: 'rgba(255,255,255,0.1)', borderRadius: 99, height: 6, overflow: 'hidden' }}>
                        <div style={{
                          height: '100%', borderRadius: 99,
                          background: pctClaimed > 80 ? '#EF4444' : pctClaimed > 50 ? '#F59E0B' : '#22C55E',
                          width: `${pctClaimed}%`,
                  ...
                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: 12, textDecoration: 'line-through', marginBottom: 2 }}>
                      {showUSD ? 'Worth 
    </div>
  );
}
...
const LIFETIME_START_DISPLAY = 199;
...
export default function PricingPage() {
  ...
  useEffect(() => {
    // Fetch how many lifetime plans have been claimed
    import('@/lib/supabaseClient').then(({ supabase }) => {
      supabase
        .from('subscriptions')
        .select('id', { count: 'exact', head: true })
        .eq('plan', 'basic_lifetime')
        .then(({ count }) => { if (count) setLifetimeClaimed(count); });
    }).catch(() => {});
  }, []);
          ...
          {[
            ...
            <div key={item.text} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <span style={{ fontFamily: 'var(--font-body)', fontSize: 12, color: '#4B5563' }}>{item.text}</span>
...
}08/year' : 'Worth ₹5,988/year'}
                    </div>
                    <div style={{ color: '#F59E0B', fontSize: 36, fontWeight: 800, lineHeight: 1 }}>
                      {showUSD ? '$499' : '₹9,999'}
                    </div>
                    ...
                    <button
                      disabled={isSoldOut}
                      onClick={() => alert('Lifetime plan launching soon! Write to support@vedicmindai.in to reserve your spot.')}
                      style={{
                        background: isSoldOut ? '#374151' : '#F59E0B',
                        color: isSoldOut ? '#9CA3AF' : '#0A1628',
                        border: 'none', borderRadius: 10,
                        padding: '11px 24px', fontWeight: 700,
                        fontSize: 14, cursor: isSoldOut ? 'not-allowed' : 'pointer',
                        whiteSpace: 'nowrap',
                      }}>
