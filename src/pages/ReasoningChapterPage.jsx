...
export default function ReasoningChapterPage() {
  ...
  useEffect(() => {
    ...
    // Free-plan users: only the one designated free chapter is reachable at
    // all, regardless of progression — send them to /pricing, not chapter 1,
    // since that's the actually useful destination for this case.
    if (plan === 'free' && !isReasoningChapterFreeAccess(chapterId)) {
      navigate('/pricing', { replace: true });
      return;
    }
    // Basic+ users: existing sequential-progression check, unchanged.
