...
export default function LiveClassPage() {
  ...
  if (loading) return <div style={{ minHeight: '100vh', background: '#F0F4FF' }}><DashboardNavbar /><p style={{ textAlign: 'center', padding: 60, color: '#6B7280' }}>{t('loadingText')}</p></div>;
  if (!liveClass) return <div style={{ minHeight: '100vh', background: '#F0F4FF' }}><DashboardNavbar /><p style={{ textAlign: 'center', padding: 60, color: '#6B7280' }}>{t('lcClassNotFound')}</p></div>;
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F0F4FF' }}>
      ...
      <main style={{ maxWidth: 720, margin: '0 auto', padding: '24px 16px 60px' }}>
        <Link to="/dashboard" style={{ color: '#6B7280', fontSize: 14, textDecoration: 'none', display: 'inline-block', marginBottom: 16 }}>{t('lcBackToDashboard')}</Link>
        ...
        <div style={{ display: 'inline-block', background: isLive ? '#FEE2E2' : isEnded ? '#F3F4F6' : '#DBEAFE', color: isLive ? '#DC2626' : isEnded ? '#6B7280' : '#1E40AF', fontSize: 12, fontWeight: 700, padding: '4px 12px', borderRadius: 99, marginBottom: 12 }}>
          {isLive ? t('lcLiveNow') : isEnded ? t('lcReplayAvailable') : t('lcUpcomingFree')}
        </div>

        <h1 style={{ fontFamily: 'var(--font-heading)', fontSize: 28, fontWeight: 700, color: '#0A1628', marginBottom: 8 }}>{liveClass.topic}</h1>
        <p style={{ color: '#4B5563', fontFamily: 'var(--font-body)', marginBottom: 4 }}>{t('lcWithTutor')} <strong>{liveClass.tutor_name}</strong></p>
        ...
        <div style={{ background: 'white', borderRadius: 16, padding: 24, boxShadow: '0 4px 20px rgba(10,22,40,0.06)' }}>
          {isLive && liveClass.youtube_live_url && (
            <>
              <p style={{ fontFamily: 'var(--font-body)', fontSize: 14, color: '#0A1628', marginBottom: 12, fontWeight: 600 }}>{t('lcLiveFreeForAll')}</p>
          ...
          {!isLive && !isEnded && (
            <>
              ...
              >
                {registered ? t('lcRegisteredBtn') : t('lcRegisterBtn')}
              </button>
            </>
          )}
          ...
          {isEnded && (
            hasReplayAccess ? (
              liveClass.youtube_replay_url ? (
                ...
                <p style={{ color: '#9CA3AF', fontFamily: 'var(--font-body)', fontSize: 14 }}>{t('lcReplayProcessing')}</p>
              )
            ) : (
              <>
                <p style={{ fontFamily: 'var(--font-body)', fontSize: 14, color: '#4B5563', marginBottom: 16 }}>
